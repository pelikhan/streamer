...
import { EditorConfig, AppAction, AppActionType, SetEditorAppAction } from "./App";
...
export function SelectMakeCodeEditor(props: { editor: string; dispatch: Dispatch<AppAction> }) {
    ...
    const onChange = (select: FormEvent<HTMLDivElement>, item?: IDropdownOption) => {
        if (item?.key)
            dispatch({ type: AppActionType.SET_EDITOR, editor: item?.key } as SetEditorAppAction)
    }
    return <Dropdown label={"Choose your editor"} options={options} selectedKey={editor} onChange={onChange} />
}

export default function MakeCodeEditor(props: { id: string; editor: string; multi?: boolean; }) {
    const { editor, id, multi } = props;
    const { editorConfigs, error, isPending, } = useEditorConfigs()
    if (!editorConfigs || isPending)
        return null;
